...
{	
	...
	Q = MatrixXd::Zero(6, 6);  //process noise
...
void Pose_ekf::predict(Vector3d gyro, Vector3d acc, double t)
{
	...
	process(gyro, acc, xdot, F, G,dt);
	
	x += xdot*dt;
	//cout<<"predict is "<<x<<endl;
...
//xdot = f(x, u);
void Pose_ekf::process(Vector3d gyro, Vector3d acc, VectorXd& xdot, MatrixXd& F, MatrixXd& G,double t)
{
	...
	//xdot.segment<3>(4) = xdot.segment<3>(7)*t/2;
...
// 	double dt = t - current_t;//imu message hz //VectorXd xdot(n_state);//16*1 predicted state
// 	MatrixXd F(n_state, n_state);//16*16
// 	MatrixXd G(n_state, 6);//G = dx/du
 
// 	process(gyros, acce, x, F, G, dt);//x += xdot*dt;
// 	x.head(4).normalize();
// 	cout<<"Predict is \n"<<x.segment<3>(4)<<endl;
// 	F = MatrixXd::Identity(n_state, n_state) + F*dt;//continous F and discrete F
// 	G = G*dt;
...
{
	pose = x.segment<7>(0);
	H = MatrixXd::Zero(7, n_state); //H is measurement matrix
	//edit H
...
void Pose_ekf::correct_slam(Vector3d pos, Quaterniond q, double t)
{
	...
	if(t < current_t) return;
	predict(this->gyro, this->acc, t); //predict
...
void Pose_ekf::correct_gravity(Vector3d acc,double t)
{
	...
	//cout<<"I am doing gravity correct"<<endl;
}